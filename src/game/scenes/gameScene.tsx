...
import { QUIT } from "../../store/gameReducer";

export class GameScene extends Phaser.Scene {
  // objects
  private bird!: Bird | null;
  private pipes!: Phaser.GameObjects.Group;
  private bg!: Phaser.GameObjects.TileSprite | null;
  ...
  update(): void {
    if (this.bg && this.bird && !this.bird.getDead()) {
      this.bg.tilePositionX -= 1;
      this.bird.update();
      this.physics.overlap(this.bird, this.pipes, this.hitPipe, undefined, this);
    } else {
      Phaser.Actions.Call(
      ...
      if (this.bird && this.bird.y > this.sys.canvas.height) {
        this.bird && this.bird.setDead(true);
        this.quit();
      }
    }
  }
  ...
  private addRowOfPipes(): void {
    ...
    for (let i = 0; i < 100; i++) {
      if (i !== hole && i !== hole + 1 && i !== hole + 2) {
        if (i === hole - 1) {
          this.addOnePipe(this.game.canvas.width, i * 60, 0, hole);
        } else if (i === hole + 3) {
          this.addOnePipe(this.game.canvas.width, i * 60, 1, hole);
        } else {
          this.addOnePipe(this.game.canvas.width, i * 60, 2, hole);
        }
      }
    }
  }
